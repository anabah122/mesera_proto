...
const tests = {
  ...
  'вёрстка приспособлена к узкому экрану'() {
    // Две колонки по 260px не помещаются на телефоне: сайдбар съедал бы
    // две трети ширины, а чат уезжал за край.
    assert.match(css, /@media \(max-width: 720px\)/, 'нет медиазапроса');
    assert.match(css, /100dvh/, '100vh на мобильных больше видимой области');
    assert.match(css, /at-dialog/, 'нет переключения список/диалог');
  },

  'запуск не может повиснуть на хранилище'() {
    // Мобильные браузеры в приватном режиме умеют не отвечать вовсе;
    // без предела ожидания приложение стояло бы на пустом экране.
    const storage = readFileSync(new URL('../frontend/storage.js', import.meta.url), 'utf8');
    assert.match(storage, /OPEN_TIMEOUT/, 'нет предела ожидания');
    assert.match(storage, /req\.onblocked/, 'блокировка открытия не обрабатывается');
    assert.match(storage, /tx\.onabort/, 'переполнение квоты не обрабатывается');
  },

  'сбой запуска виден на экране'() {
    // На телефоне консоли нет: пустая страница не объясняет ничего.
    const app = readFileSync(new URL('../frontend/app.js', import.meta.url), 'utf8');
    assert.match(app, /function fail\(/, 'нет экрана ошибки');
    assert.match(app, /start\(session\)\.catch\(fail\)/, 'сбой запуска не показывается');
    assert.match(css, /\.fail\s*{/, 'экран ошибки не оформлен');
  },

  'нехватка места не роняет переписку'() {
    const store = readFileSync(new URL('../frontend/store.js', import.meta.url), 'utf8');
    assert.match(store, /try \{[\s\S]{0,120}_storage\.put/,
      'запись в базу должна быть под try: сообщение важнее кеша');
  },
